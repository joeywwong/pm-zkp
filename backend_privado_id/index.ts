...
import { ProofGenerationOptions } from './node_modules/@0xpolygonid/js-sdk/dist/types/proof/provers/inputs-generator';
dotenv.config();
...
function createHealthCredential(did: core.DID) {
  const credentialRequest: CredentialRequest = {
    credentialSchema:
...
function createHealthCredentialRequest(
  circuitId: CircuitId,
  credentialRequest: CredentialRequest
): ZeroKnowledgeProofRequest {
  const proofReqSig: ZeroKnowledgeProofRequest = {
    ...
    query: {
      ...
      type: credentialRequest.type,
      context:
        'https://api.jsonbin.io/v3/b/67aec79cad19ca34f8030693?meta=false',
      credentialSubject: {
        restingHeartRate: {
          $lt: 90
        }
...
async function generateHealthProofs(useMongoStore = false) {
  ...
  const proofReqSig: ZeroKnowledgeProofRequest = createHealthCredentialRequest(
    CircuitId.AtomicQuerySigV2OnChain,
    credentialRequest
  );

  const opts: ProofGenerationOptions = {
...
async function main(choice: string) {
  switch (choice) {
    ...
    case 'generateProofs':
      ...
      break;
    case 'generateHealthProofs':
      await generateHealthProofs();
      break;
