...
export async function main(type: string, attribute: string, schemaOrRaw: string, operatorStr: string, valueParam: number, tokenID: number, contextParam: string) {
  try {
    ...
    // Generate a unique requestId using current timestamp and check for existence
    let requestId: number;
    while (true) {
      requestId = Math.floor(Date.now() / 1000);
      const requestIdExists = await verifier.requestIdExists(requestId);
      if (!requestIdExists) break;
      // If exists, loop again to get a new timestamp
    }
    ...
    const invokeRequestMetadataKYCAgeCredential = {
      ...
      body: {
        reason: "spending condition for the transfer of token " + tokenID,
        transaction_data: {
